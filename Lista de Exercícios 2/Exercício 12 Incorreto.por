	...
	funcao inicio()
	{
		...
		para(i = 0; i <= 4; i++){
			se(result[i] != 0){
				...
				} senao se(result[i] < primeiroLugar e result[i] > ultimoLugar e result[i] > segundoLugar){
					segundoLugar = result[i]
				} senao se(result[i] < ultimoLugar e result[i] < primeiroLugar e result[i] < segundoLugar){
